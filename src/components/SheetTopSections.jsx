...
// ── STYLES ────────────────────────────────────────────────────────────────────
const lbl = { fontSize:'.6rem', letterSpacing:'.16em', color:'var(--text3)', textTransform:'uppercase', fontFamily:'Georgia, serif', display:'block', marginBottom:2 }
...
const selectSt = { background:'var(--surface2)', border:'1px solid var(--border2)', color:'var(--text)', borderRadius:4, padding:'3px 6px', fontFamily:'Georgia, serif', fontSize:'.72rem', cursor:'pointer', width:'100%' }
...
// ── ATTR ROW ──────────────────────────────────────────────────────────────────
function AttrRow({ abbr, current, checkMod, derivedLabel, derivedValue, detailContent }) {
  const [expanded, setExpanded] = useState(false)
  const cc = checkMod >= 0 ? 'var(--text2)' : '#c94a4a'
  return (
    <>
      <div style={{ display:'flex', alignItems:'center', ...rowDiv, minHeight:44 }}>
        <div onClick={() => setExpanded(!expanded)} style={{ display:'flex', alignItems:'center', gap:8, padding:'8px 12px', cursor:'pointer', flex:'0 0 auto' }}>
          <span style={{ fontSize:'.85rem', color:'var(--gold)', fontFamily:'Georgia, serif', fontWeight:600, minWidth:28 }}>{abbr}</span>
          <span style={{ ...val, fontSize:'1.1rem' }}>{current}</span>
          <span style={{ fontSize:'.82rem', color:cc, fontFamily:'Georgia, serif' }}>{checkMod>=0?'+':''}{checkMod}</span>
...
function SessionRow({ stats, character, offHand, stance, onOffHandChange, onStanceChange }) {
  return (
...
export function AttributeBlock({ stats, character, onUpdateCharacter, offHand, stance, onOffHandChange, onStanceChange }) {
  ...
  const dexExp = lookupAttr('dexterity', attrs.dex.effective, 'expertise')
  const dexPR  = lookupAttr('dexterity', attrs.dex.effective, 'precision')
  const awEv   = lookupAttr('awareness',  attrs.aw.effective,  'evasionBonus')
  ...
  return (
    <div style={{ background:'var(--surface)', border:'1px solid var(--border)', borderRadius:8, overflow:'hidden' }}>
      ...
      <AttrRow abbr="CON" current={attrs.con.effective} checkMod={attrs.con.checkMod}
        ...
        detailContent={<DI label="Rolled CON" value={getBase('con')} />} />
      ...
      <SessionRow stats={stats} character={character} offHand={offHand} stance={stance} onOffHandChange={onOffHandChange} onStanceChange={onStanceChange} />
    </div>
  )
}
...
const DCOLS = '150px 120px 120px 120px 150px minmax(100px, 330px) 100px'
  const MCOLS = '48px 90px 80px 1fr'
  ...
  const renderRow = (loc, label, isHelm) => {
    ...
    const maxHPVal = getMaxHP(loc, maxHP)
    const curHPVal = curHP[loc] ?? maxHPVal
    const arVal    = getARForLoc(code, loc, helmCode)
    ...
    if (isDesktop) {
      return (
        <div key={loc} style={{ display:'grid', gridTemplateColumns:grid, ...rowDiv, alignItems:'center' }}>
          {locCell(label)}
          {dataCell(<ClickEdit value={curHPVal} onChange={v=>updateHP(loc,v)} max={maxHPVal} />)}
          {dataCell(<span style={dimVal}>{maxHPVal}</span>)}
        ...
        <div key={loc} style={{ display:'grid', gridTemplateColumns:grid, ...rowDiv, alignItems:'center' }}>
          ...
          {dataCell(
            <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:1 }}>
              <ClickEdit value={curHPVal} onChange={v=>updateHP(loc,v)} max={maxHPVal} fontSize=".9rem" />
              {sep}
              <span style={{ fontSize:'.72rem', color:'var(--text3)', fontFamily:'Georgia, serif' }}>{maxHPVal}</span>
            </div>
          )}
          {dataCell(
            <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:1 }}>
  ...
  // Global row — 1 field per column, no label column
